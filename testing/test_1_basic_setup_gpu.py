...
def test_docker_configure(playwright: Playwright) -> None:
    ...
    expect(page.get_by_label("Docker Host").get_by_label("value")).to_be_visible(timeout=10000)
    time.sleep(1.0)
    ...
    time.sleep(1.0)
    # page.get_by_label("Disabled").click() # Removed due to UI bug
    time.sleep(1.0)
...
def test_start_container(playwright: Playwright, containername) -> None:
    ...
    expect(page.get_by_label("none")).to_be_enabled(timeout=60000)
    page.get_by_label("none").click()
    page.get_by_role("option", name="nvidia").click()
...
def test_check_jupyter(playwright: Playwright, containername) -> None:
    ...
    expect(page.get_by_text(">")).to_be_visible(timeout=60000)
...
def test_barebones_model(playwright: Playwright, containername) -> None:
    ...
    page2.goto(f"http://{instance_location}:8000/en-GB/app/search/search?q=%7C%20makeresults%20count%3D10%20%0A%7C%20streamstats%20c%20as%20i%20%0A%7C%20eval%20i%3Di-1%20%0A%7C%20fit%20AITKContainer%20algo%3Dbarebone_template%20i%20%0A%7C%20eval%20test%3Di-predicted_index%20%0A%7C%20stats%20sum(test)%20as%20test%20%0A%7C%20eval%20test%3Dif(test%3D%3D0%2C%22SUCCESS%22%2C%22FAIL%22)&display.page.search.mode=fast&dispatch.sample_ratio=1&workload_pool=&earliest=-24h%40h&latest=now&display.page.search.tab=statistics&display.general.type=statistics")
